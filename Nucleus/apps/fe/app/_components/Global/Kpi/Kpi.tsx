...
function getTrendClasses(trend: KpiProps['trend']): string {
  const map: Record<NonNullable<KpiProps['trend']>, string> = {
    up: 'text-emerald-700',
    down: 'text-rose-700',
    steady: 'text-slate-500 dark:text-slate-400',
  }
  if (trend === undefined) {
    ...
    >
      <header className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          {icon ? (
            <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-100 dark:bg-slate-950 text-slate-600 dark:text-slate-400">
              {icon}
            </div>
          ) : null}
      ...
      <footer className="flex items-baseline gap-2 text-sm">
        {subValue ? <span className="text-slate-500 dark:text-slate-400">{subValue}</span> : null}
        {trendLabel ? (
