...
export function AuditPlannerPanel(props: {
  ...
  const { locations, teams, audits, onCreate, onUpdateStatus, onDelete } = props;
  ...
  return (
    <section className="rounded-xl border border-slate-800 bg-slate-900/80">
      <div className="border-b border-slate-800 px-4 py-3">
        <h2 className="text-sm font-semibold text-slate-100">Denetim Planlama</h2>
        <p className="mt-1 text-xs text-slate-400">Lokasyon + ekip seçerek plan oluştur.</p>
      </div>

      <div className="p-4 space-y-5">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="space-y-1">
            <label htmlFor="plannedDate" className="block text-xs font-medium text-slate-300">
              Denetim Tarihi
            </label>
          ...
          <div className="space-y-1">
            <label htmlFor="locationId" className="block text-xs font-medium text-slate-300">
              Lokasyon
            </label>
          ...
          <div className="space-y-1">
            <label htmlFor="assignedTeamId" className="block text-xs font-medium text-slate-300">
              Atanan Ekip
            </label>
          ...
          <div className="space-y-1">
            <label htmlFor="note" className="block text-xs font-medium text-slate-300">
              Not
            </label>
        ...
        <div className="flex justify-end">
          <PrimaryButton onClick={submit} disabled={!plannedDate || !locationId || !assignedTeamId}>
            Denetimi Oluştur
          </PrimaryButton>
        </div>
        ...
        <div className="overflow-hidden rounded-lg border border-slate-800">
          ...
          {audits.length === 0 ? (
            <div className="px-3 py-6 text-sm text-slate-400">Henüz plan yok.</div>
          ) : (
            audits.map((a) => {
              const loc = locations.find((x) => x.id === a.locationId)?.name ?? "-";
              ...
              return (
                ...
                >
                  <div className="col-span-2 text-sm text-slate-200">{a.plannedDate}</div>
                  <div className="col-span-4 text-sm text-slate-100">{loc}</div>
                  <div className="col-span-3 text-sm text-slate-300">{team}</div>
                  ...
                  <div className="col-span-2 flex justify-end gap-2">
                    ...
                    <DangerButton className="py-1.5" onClick={() => onDelete(a.id)}>
                      Sil
                    </DangerButton>
                  </div>
